...
for bin_label in lxy_labels:
    ...
    fig.savefig(f'{out_dir}/FeatImp_lxy_{bin_label}.png', dpi=150, bbox_inches='tight')
    plt.close(fig)

    # ROC curves — one per (ctau, mA, mpi) point
    # For non-conditional: signal of that param + all background in test set
    ...
    for ctau_val, mA_val, mpi_val in param_grid:
        ...
        ax.set_title(f'ROC ctau={ctau_val}mm mA={mA_val} mpi={mpi_val} lxy={bin_label}')
        ax.legend(loc='lower right')
        fig.savefig(
            f'{out_dir}/ROC_ctau{_flabel(ctau_val)}_mA{_flabel(mA_val)}_mpi{_flabel(mpi_val)}_lxy_{bin_label}.png',
...
ax.tick_params(axis='y', labelsize=8)
fig.savefig(f'{out_dir}/FeatImp_global.png', dpi=150, bbox_inches='tight')
plt.close(fig)

# ROC curves per (ctau, mA, mpi) — global BDT
...
for ctau_val, mA_val, mpi_val in param_grid:
    ...
    if use_conditional:
        ...
        bkg_mask  = df_test_meta_g['label'] == 0
        eval_mask = sig_mask | bkg_mask
    ...
    fig, ax = plt.subplots(figsize=(6, 6), constrained_layout=True)
    ax.plot(fpr_g, tpr_g, label=f'AUC={auc_g:.3f}')
    ax.plot([0, 1], [0, 1], 'k--', alpha=0.5)
    ax.set_xlabel('False Positive Rate')
    ax.set_ylabel('True Positive Rate')
    ax.set_title(f'ROC Global ctau={ctau_val}mm mA={mA_val} mpi={mpi_val}')
    ax.legend(loc='lower right')
    fig.savefig(
        f'{out_dir}/ROC_global_ctau{_flabel(ctau_val)}_mA{_flabel(mA_val)}_mpi{_flabel(mpi_val)}.png',
        dpi=150, bbox_inches='tight',
    )
    plt.close(fig)
